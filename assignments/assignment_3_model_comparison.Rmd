...
ggplot(data=source_data, aes(x=weight))+geom_histogram()
ggplot(data=source_data, aes(x=IQ))+geom_histogram()
ggplot(data=source_data, aes(x=household_income))+geom_histogram()
...
simple_model = lm(formula = pain ~ age + sex, data= source_data)
...
cleaned_simple_model = lm(pain~age+sex, data = cleaned_data)
summary(cleaned_simple_model)
```
...
library(olsrr)
plot(cleaned_simple_model,2)
ols_plot_resid_hist(cleaned_simple_model)
...
```{r}
library(car)
vif(cleaned_simple_model)

#Vif scores are not reaching any thresholds, we can assume that there is no multicollinearity
```
